...
const TimeEntry = require('../../models/timeEntry'); // Модель для Time Entries
      ...
      try {
        ...
        const timeEntry = await TimeEntry.findOne({
          ...
          include: {
            ...
            attributes: ['id', 'title', 'assignee_id'],
          },
        });
        ...
        // Перевіряємо, чи користувач є виконавцем завдання
        if (timeEntry.Task.assignee_id !== req.user.id) {
          return res.status(403).json({ error: 'Access denied. You are not the assignee of this task.' });
        }
